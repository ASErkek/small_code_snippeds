 ...
 * Constructor
 */
AS5048A::AS5048A(byte HSPI_SS, byte HSPI_MISO, byte HSPI_MOSI, byte HSPI_SCLK)
	: _HSPI_SS(HSPI_SS), _HSPI_MISO(HSPI_MISO), _HSPI_MOSI(HSPI_MOSI), _HSPI_SCLK(HSPI_SCLK), errorFlag(false), ocfFlag(false), position(0), debug(debug)
{
}
...
void AS5048A::begin()
{
	...
	//setup pins
	pinMode(this->_HSPI_SS, OUTPUT);

	//SPI has an internal SPI-device counter, it is possible to call "begin()" from different devices
	hspi->begin(this->_HSPI_SCLK, this->_HSPI_MISO, this->_HSPI_MOSI, this->_HSPI_SS); //SCLK, MISO, MOSI, SS
}
 ...
 * Get diagnostic
 */
std::string AS5048A::getDiagnostic()
{
	uint16_t data = AS5048A::getState();
...
uint16_t AS5048A::read(uint16_t registerAddress)
{
	...
	hspi->beginTransaction(this->settings);

	//Send the command
	digitalWrite(this->_HSPI_SS, LOW);
	hspi->transfer16(command);
	digitalWrite(this->_HSPI_SS, HIGH);

	delay(this->esp32_delay);

	//Now read the response
	digitalWrite(this->_HSPI_SS, LOW);
	uint16_t response = hspi->transfer16(0x00);
	digitalWrite(this->_HSPI_SS, HIGH);
...
uint16_t AS5048A::write(uint16_t registerAddress, uint16_t data)
{
	...
	digitalWrite(this->_HSPI_SS, LOW);
	uint16_t response = hspi->transfer16(0x0000);
	digitalWrite(this->_HSPI_SS, HIGH);
